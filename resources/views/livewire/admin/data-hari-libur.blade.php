...
>
                ...
                <div class="group relative overflow-hidden rounded-2xl border border-gray-200 bg-white p-4 shadow-sm ring-1 ring-black/5"
                    ...
                    <div class="flex items-start gap-3">
                        <div class="relative h-12 w-12 shrink-0 rounded-xl bg-gradient-to-br from-emerald-500 to-green-500 text-white ring-2 shadow-sm grid place-items-center">
                            <img src="{{ asset('images/calendar_icon.svg') }}" class="h-7 w-7" alt="libur">
                        </div>
                    ...
                    <div class="mt-4">
                        <div class="grid grid-cols-2 gap-2">
                            <button type="button"
                                ...
                                @click="$dispatch('modal-edit-open'); $wire.openEditModal({{ $h->id }})">
                                Edit
                            </button>
                            <button type="button"
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($items as $h)
                        <tr class="hover:bg-gray-50" wire:key="row-{{ $h->id }}">
                            <td class="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{{ $h->id }}</td>
    ...
    {{-- Styles untuk x-cloak (dipindah ke dalam root agar tidak jadi root terpisah) --}}
    @once
