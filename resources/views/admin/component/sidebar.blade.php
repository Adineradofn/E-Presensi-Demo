...
@persist('sidebar')
    ...
    >
        ...
        <button 
            ...
            x-transition
        >
            <img src="{{ asset('images/hamburger_icon.svg') }}" class="h-5 w-5" alt="Open menu">
        </button>

        {{-- Sidebar DESKTOP (md+): selalu terlihat, ditempel kiri layar --}}
        <aside 
            class="hidden md:block w-64 h-screen bg-white fixed left-0 top-0 z-30 border-r border-gray-200"
            role="navigation" 
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.dashboard') }}" wire:navigate wire:current="{{ $active }}"
                   class="{{ $baseLink }} {{ $hover }} {{ $normal }}">
                    <img src="{{ asset('images/icon_dashboard.svg') }}" class="h-5 w-5" alt="Dashboard">
                    <span>Dashboard</span>
                </a>

                <a href="{{ route('admin.data.karyawan') }}" wire:navigate wire:current="{{ $active }}"
                   class="{{ $baseLink }} {{ $hover }} {{ $normal }}">
                    <img src="{{ asset('images/icon_karyawan.svg') }}" class="h-5 w-5" alt="Data Karyawan">
                    <span>Data Karyawan</span>
                </a>

                <a href="{{ route('admin.data.presensi') }}" wire:navigate wire:current="{{ $active }}"
                   class="{{ $baseLink }} {{ $hover }} {{ $normal }}">
                    <img src="{{ asset('images/icon_kehadiran.svg') }}" class="h-5 w-5" alt="Data Presensi">
                ...
                <a href="{{ route('admin.data.rekap-presensi') }}" wire:navigate wire:current="{{ $active }}"
                   class="{{ $baseLink }} {{ $hover }} {{ $normal }}">
                    <img src="{{ asset('images/document_icon.svg') }}" class="h-5 w-5" alt="Rekap Presensi">
                    <span>Rekap Presensi</span>
                </a>

                <a href="{{ route('admin.pengajuan-izin') }}" wire:navigate wire:current="{{ $active }}"
                   class="{{ $baseLink }} {{ $hover }} {{ $normal }}">
                    <img src="{{ asset('images/icon_izin.svg') }}" class="h-5 w-5" alt="Data Pengajuan Izin">
        ...
        <div 
            ...
            class="fixed inset-0 z-40 md:hidden bg-black/15" 
            @click="open = false" 
            aria-hidden="true"
        ></div>
